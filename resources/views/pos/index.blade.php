...
<body class="font-sans antialiased text-gray-900 bg-gray-50 flex h-screen overflow-hidden" x-data="posApp()">
    ...
    <div class="w-20 bg-white border-r flex flex-col items-center py-6 shadow-sm z-10">
        <div
            class="w-10 h-10 bg-blue-600 text-white rounded-lg flex items-center justify-center font-bold text-xl mb-8">
            P
        </div>
        <nav class="flex-1 flex flex-col gap-6">
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-auto">
                @csrf
                <button type="submit" class="p-3 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"
                    title="Logout">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex-1 p-8 overflow-y-auto">
            ...
            <div class="flex gap-3 mb-6 overflow-x-auto pb-2">
                <button @click="activeCategory = null"
                    :class="activeCategory === null ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'"
                    class="px-4 py-2 border rounded-full text-sm font-medium transition whitespace-nowrap">All
                    Items</button>
                @foreach($categories as $cat)
                    <button @click="activeCategory = {{ $cat->id }}"
                        :class="activeCategory === {{ $cat->id }} ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'"
                        class="px-4 py-2 border rounded-full text-sm font-medium transition whitespace-nowrap">{{ $cat->name }}</button>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <template x-for="product in filteredProducts" :key="product.id">
                    <div @click="addToCart(product)"
                        class="bg-white rounded-xl shadow-sm border p-4 cursor-pointer hover:border-blue-500 hover:shadow-md transition">
                        <div
                            class="w-full h-32 bg-gray-100 rounded-lg mb-4 flex items-center justify-center text-gray-400 overflow-hidden">
    ...
    <aside class="w-96 bg-white border-l shadow-xl flex flex-col h-full z-20">
        <div class="p-6 border-b">
            ...
            <p class="text-sm text-gray-500 mt-1">Order #<span
                    x-text="Math.floor(Math.random() * 90000) + 10000"></span></p>
        </div>
                ...
                <div class="flex items-center justify-between border-b pb-4">
                    ...
                    <div class="flex items-center gap-3">
                        <button @click="decreaseQty(item)"
                            class="w-8 h-8 rounded bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-gray-200">-</button>
                        <span class="font-medium" x-text="item.qty"></span>
                        <button @click="increaseQty(item)"
                            class="w-8 h-8 rounded bg-blue-100 flex items-center justify-center text-blue-600 hover:bg-blue-200">+</button>
                    </div>
                </div>
            </template>
        ...
        <div class="p-6 bg-gray-50 border-t">
            ...
            <div class="flex gap-2 mb-4">
                <button @click="paymentMethod = 'cash'"
                    :class="paymentMethod === 'cash' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 border'"
                    class="flex-1 py-2 rounded-lg text-sm font-bold transition">Cash</button>
                <button @click="paymentMethod = 'qris'"
                    :class="paymentMethod === 'qris' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 border'"
                    class="flex-1 py-2 rounded-lg text-sm font-bold transition">QRIS / Tf</button>
            </div>

            <!-- Payment Action -->
            <button @click="checkout()" :disabled="cart.length === 0 || isProcessing"
                :class="cart.length === 0 || isProcessing ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'"
                class="w-full text-white font-bold py-4 rounded-xl shadow-md transition text-lg flex items-center justify-center gap-2">
                    ...
                    try {
                        ...
                        if (data.success) {
                            if (this.paymentMethod === 'cash') {
                                ...
                                // isProcessing tetap true selama popup Midtrans terbuka
                                // agar tombol "Pay Now" tidak bisa diklik lagi
                                ...
                                // Jangan set isProcessing = false di sini,
                                // biarkan callback di atas yang mengaturnya
                                return;
                            }
                        } else {
